...
export default function ActivityList({activities, dispatch}: ActivityListProp) {
    ...
    return (
    <>
        <h2 className="text-4xl font-bold text-slate-600 text-center">Food and Excercices</h2>

        {isEmptyActivities ? <p className="text-center">There is no activities yet</p> : 

        activities.map(activity => (
            <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between">
                <div className="space-y-2 relative">
                    <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold
                        ${activity.category === 1 ? 'bg-orange-300': 'bg-lime-500'}`}>
